...
dirname = os.path.join(os.getcwd(), "./assets/resized_images")
...
epochs = (
    25  # Cantidad de iteraciones completas al conjunto de imagenes de entrenamiento
)
...
riesgo_model.add(
    Conv2D(32, (3, 3), activation="linear", padding="same", input_shape=(128, 128, 3))
)
...
for i, correct in enumerate(correct[0:9]):
    ...
    plt.title(
        "{}, {}".format(sriesgos[predicted_classes[correct]], sriesgos[test_Y[correct]])
    )
...
for i, incorrect in enumerate(incorrect[0:9]):
    plt.subplot(3, 3, i + 1)
    plt.imshow(
        test_X[incorrect].reshape(128, 128, 3), cmap="gray", interpolation="none"
    )
    plt.title(
        "{}, {}".format(
            sriesgos[predicted_classes[incorrect]], sriesgos[test_Y[incorrect]]
        )
    )
    plt.tight_layout()
